...
proc vtk_output_node_disp {chan nombre} {
  set nids [getNodeTags]
  set num_of_nodes [llength $nids]
  set cur_time [getTime]
  set ndf [get_ndf]

  puts $chan "VECTORS $nombre float"
  foreach n $nids {
...
proc vtk_output_ele_resp_stresses {chan nombre} {
  set nelems [getEleTags]
  set num_of_elements [llength $nelems]
  set cur_time [getTime]
  set ndf [get_ndf]
  ...
  puts $chan "$nombre 6 $num_of_elements float"
  foreach n $nelems {
    set resp [string trim [eleResponse $n material 9 stress]]
    set resp [regexp -inline -all {\S+} $resp]
    while {[llength $resp] < 6} { lappend resp 0.0 }
...
proc vtk_output_ele_resp_strains {chan nombre} {
  set nelems [getEleTags]
  set num_of_elements [llength $nelems]
  set cur_time [getTime]
  set ndf [get_ndf]

  puts $chan "FIELDS ele_resp_$cur_time 1"
  puts $chan "$nombre 6 $num_of_elements float"
  foreach n $nelems {
    set resp [string trim [eleResponse $n material 9 strain]]
    set resp [regexp -inline -all {\S+} $resp]
    while {[llength $resp] < 6} { lappend resp 0.0 }
    if {$ndf > 3} { set resp [lrange $resp 0 2] }
    puts $chan $resp
