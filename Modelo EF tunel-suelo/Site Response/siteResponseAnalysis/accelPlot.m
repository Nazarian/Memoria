...
desps=load('displacement.out');
[m,n]=size(desps);

desps=desps(:,1:2:end);
desps=[zeros(m,1),desps];
[m,n]=size(desps);
maximos=zeros(n-1,1);

for i=1:length(maximos)
   for j=1:m
      if (abs(desps(j,i+1)-desps(j,i))>maximos(i))
          maximos(i)=abs(desps(j,i+1)-desps(j,i));
      end
   end
end

maximos=maximos(4:end);
maximos=maximos(1:2:end)*4;
maximos=smooth(maximos);
zetas=-48.0:0.5:0;
figure
plot(maximos,zetas)
xlabel('\gamma_{max}')
